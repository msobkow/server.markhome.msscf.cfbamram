...
// Description: Java 11 in-memory RAM DbIO implementation for UuidGen.
	...
	implements ICFBamUuidGenTable
{
	...
	public CFBamRamUuidGenTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamUuidGenBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a86e" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a86e" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamUuidGenBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamUuidGenBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		long SchemaDefId )
	{
		...
		CFBamUuidGenBuff buff;
		ArrayList<CFBamUuidGenBuff> filteredList = new ArrayList<CFBamUuidGenBuff>();
		CFBamUuidGenBuff[] buffList = readDerivedBySchemaIdx( Authorization,
			TenantId,
			SchemaDefId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a851" ) ) {
				filteredList.add( (CFBamUuidGenBuff)buff );
			}
		}
		return( filteredList.toArray( new CFBamUuidGenBuff[0] ) );
	...
	/**
	 *	Read a page array of the specific UuidGen buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamUuidGenBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamUuidGenBuff)editCur );
	}
		...
		CFBamUuidTypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableUuidGen().readDerivedByIdIdx( Authorization,
