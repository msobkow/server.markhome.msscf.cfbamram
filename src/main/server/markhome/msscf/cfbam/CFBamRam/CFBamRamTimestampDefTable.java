...
// Description: Java 11 in-memory RAM DbIO implementation for TimestampDef.
	...
	implements ICFBamTimestampDefTable
{
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a846" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long TenantId )
	{
		...
		CFBamTimestampDefBuff buff;
		ArrayList<CFBamTimestampDefBuff> filteredList = new ArrayList<CFBamTimestampDefBuff>();
		CFBamTimestampDefBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		int revision )
	{
		...
		{
			return( (CFBamTimestampDefBuff)cur );
		}
