...
// Description: Java 11 in-memory RAM DbIO implementation for TZTimestampType.
	...
	implements ICFBamTZTimestampTypeTable
{
	...
	public CFBamRamTZTimestampTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTZTimestampTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a83f" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a83f" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTZTimestampTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTZTimestampTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTZTimestampTypeBuff buff;
		ArrayList<CFBamTZTimestampTypeBuff> filteredList = new ArrayList<CFBamTZTimestampTypeBuff>();
		CFBamTZTimestampTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TZTimestampType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamTZTimestampTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		{
			return( (CFBamTZTimestampTypeBuff)cur );
		}
		...
		return( (CFBamTZTimestampTypeBuff)editCur );
	}
