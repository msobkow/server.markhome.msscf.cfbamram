...
// Description: Java 11 in-memory RAM DbIO implementation for DateType.
	...
	implements ICFBamDateTypeTable
{
	...
	public CFBamRamDateTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamDateTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a816" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a816" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamDateTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamDateTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamDateTypeBuff buff;
		ArrayList<CFBamDateTypeBuff> filteredList = new ArrayList<CFBamDateTypeBuff>();
		CFBamDateTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific DateType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamDateTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamDateTypeBuff)editCur );
	}
		...
		CFBamDateTypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableDateType().readDerivedByIdIdx( Authorization,
