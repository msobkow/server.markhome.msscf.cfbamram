...
// Description: Java 11 in-memory RAM DbIO implementation for TZTimeDef.
	...
	implements ICFBamTZTimeDefTable
{
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a83c" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long TenantId )
	{
		...
		CFBamTZTimeDefBuff buff;
		ArrayList<CFBamTZTimeDefBuff> filteredList = new ArrayList<CFBamTZTimeDefBuff>();
		CFBamTZTimeDefBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
