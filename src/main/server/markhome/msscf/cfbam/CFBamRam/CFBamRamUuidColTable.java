...
// Description: Java 11 in-memory RAM DbIO implementation for UuidCol.
	...
	implements ICFBamUuidColTable
{
	...
	public CFBamRamUuidColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamUuidColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a86d" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a86d" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamUuidColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamUuidColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamUuidColBuff buff;
		ArrayList<CFBamUuidColBuff> filteredList = new ArrayList<CFBamUuidColBuff>();
		CFBamUuidColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific UuidCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamUuidColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamUuidColBuff)editCur );
	}
		...
		CFBamUuidColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableUuidCol().readDerivedByIdIdx( Authorization,
