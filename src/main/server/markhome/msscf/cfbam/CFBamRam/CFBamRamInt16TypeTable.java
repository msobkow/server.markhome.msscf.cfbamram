...
// Description: Java 11 in-memory RAM DbIO implementation for Int16Type.
	...
	implements ICFBamInt16TypeTable
{
	...
	public CFBamRamInt16TypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamInt16TypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a824" ) ) {
		...
		long Id )
	{
		...
		CFBamInt16TypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamInt16TypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamInt16TypeBuff buff;
		ArrayList<CFBamInt16TypeBuff> filteredList = new ArrayList<CFBamInt16TypeBuff>();
		CFBamInt16TypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific Int16Type buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamInt16TypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamInt16TypeBuff)editCur );
	}
		...
		CFBamInt16TypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableInt16Type().readDerivedByIdIdx( Authorization,
				cur.getRequiredTenantId(),
				cur.getRequiredId() );
			String subClassCode = cur.getClassCode();
			if( "a824".equals( subClassCode ) ) {
