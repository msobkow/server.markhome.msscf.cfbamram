...
// Description: Java 11 in-memory RAM DbIO implementation for BlobCol.
	...
	implements ICFBamBlobColTable
{
	...
	public CFBamRamBlobColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamBlobColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a852" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a852" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamBlobColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamBlobColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamBlobColBuff buff;
		ArrayList<CFBamBlobColBuff> filteredList = new ArrayList<CFBamBlobColBuff>();
		CFBamBlobColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific BlobCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamBlobColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamBlobColBuff)editCur );
	}
		...
		CFBamBlobColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableBlobCol().readDerivedByIdIdx( Authorization,
