...
// Description: Java 11 in-memory RAM DbIO implementation for NumberType.
	...
	implements ICFBamNumberTypeTable
{
	...
	public CFBamRamNumberTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamNumberTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a82e" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a82e" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamNumberTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamNumberTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamNumberTypeBuff buff;
		ArrayList<CFBamNumberTypeBuff> filteredList = new ArrayList<CFBamNumberTypeBuff>();
		CFBamNumberTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific NumberType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamNumberTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		{
			return( (CFBamNumberTypeBuff)cur );
		}
		...
		return( (CFBamNumberTypeBuff)editCur );
	}
		...
		CFBamNumberTypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableNumberType().readDerivedByIdIdx( Authorization,
