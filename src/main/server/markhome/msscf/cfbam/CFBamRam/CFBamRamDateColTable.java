...
// Description: Java 11 in-memory RAM DbIO implementation for DateCol.
	...
	implements ICFBamDateColTable
{
	...
	public CFBamRamDateColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamDateColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a854" ) ) {
		...
		long Id )
	{
		...
		CFBamDateColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamDateColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamDateColBuff buff;
		ArrayList<CFBamDateColBuff> filteredList = new ArrayList<CFBamDateColBuff>();
		CFBamDateColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific DateCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamDateColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamDateColBuff)editCur );
	}
		...
		CFBamDateColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableDateCol().readDerivedByIdIdx( Authorization,
