...
// Description: Java 11 in-memory RAM DbIO implementation for FloatType.
	...
	implements ICFBamFloatTypeTable
{
	...
	public CFBamRamFloatTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamFloatTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a820" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a820" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamFloatTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamFloatTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamFloatTypeBuff buff;
		ArrayList<CFBamFloatTypeBuff> filteredList = new ArrayList<CFBamFloatTypeBuff>();
		CFBamFloatTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific FloatType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamFloatTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamFloatTypeBuff)editCur );
	}
