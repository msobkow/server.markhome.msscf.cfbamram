...
// Description: Java 11 in-memory RAM DbIO implementation for EnumDef.
	...
	implements ICFBamEnumDefTable
{
		...
		long TenantId )
	{
		...
		CFBamEnumDefBuff buff;
		ArrayList<CFBamEnumDefBuff> filteredList = new ArrayList<CFBamEnumDefBuff>();
		CFBamEnumDefBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
