...
// Description: Java 11 in-memory RAM DbIO implementation for TimestampType.
	...
	implements ICFBamTimestampTypeTable
{
	...
	public CFBamRamTimestampTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTimestampTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a847" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a847" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTimestampTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTimestampTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTimestampTypeBuff buff;
		ArrayList<CFBamTimestampTypeBuff> filteredList = new ArrayList<CFBamTimestampTypeBuff>();
		CFBamTimestampTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TimestampType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamTimestampTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		{
			return( (CFBamTimestampTypeBuff)cur );
		}
		...
		return( (CFBamTimestampTypeBuff)editCur );
	}
