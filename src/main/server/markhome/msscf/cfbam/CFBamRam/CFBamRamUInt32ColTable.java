...
// Description: Java 11 in-memory RAM DbIO implementation for UInt32Col.
	...
	implements ICFBamUInt32ColTable
{
	...
	public CFBamRamUInt32ColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamUInt32ColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a86b" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a86b" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamUInt32ColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamUInt32ColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamUInt32ColBuff buff;
		ArrayList<CFBamUInt32ColBuff> filteredList = new ArrayList<CFBamUInt32ColBuff>();
		CFBamUInt32ColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific UInt32Col buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamUInt32ColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		CFBamUInt32ColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableUInt32Col().readDerivedByIdIdx( Authorization,
