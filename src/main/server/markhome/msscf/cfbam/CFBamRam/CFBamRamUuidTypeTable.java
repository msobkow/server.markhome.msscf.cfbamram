...
// Description: Java 11 in-memory RAM DbIO implementation for UuidType.
	...
	implements ICFBamUuidTypeTable
{
	...
	public CFBamRamUuidTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamUuidTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a851" ) ) {
		...
		long Id )
	{
		...
		CFBamUuidTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamUuidTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamUuidTypeBuff buff;
		ArrayList<CFBamUuidTypeBuff> filteredList = new ArrayList<CFBamUuidTypeBuff>();
		CFBamUuidTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific UuidType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamUuidTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamUuidTypeBuff)editCur );
	}
		...
		CFBamUuidTypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableUuidType().readDerivedByIdIdx( Authorization,
				cur.getRequiredTenantId(),
				cur.getRequiredId() );
			String subClassCode = cur.getClassCode();
			if( "a851".equals( subClassCode ) ) {
