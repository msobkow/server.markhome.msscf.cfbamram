...
// Description: Java 11 in-memory RAM DbIO implementation for FloatCol.
	...
	implements ICFBamFloatColTable
{
	...
	public CFBamRamFloatColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		long Id )
	{
		...
		CFBamFloatColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamFloatColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamFloatColBuff buff;
		ArrayList<CFBamFloatColBuff> filteredList = new ArrayList<CFBamFloatColBuff>();
		CFBamFloatColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific FloatCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamFloatColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamFloatColBuff)editCur );
	}
		...
		CFBamFloatColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableFloatCol().readDerivedByIdIdx( Authorization,
