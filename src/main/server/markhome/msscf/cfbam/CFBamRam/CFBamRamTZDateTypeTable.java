...
// Description: Java 11 in-memory RAM DbIO implementation for TZDateType.
	...
	implements ICFBamTZDateTypeTable
{
		...
		CFBamTZDateTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a83b" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a83b" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long TenantId )
	{
		...
		CFBamTZDateTypeBuff buff;
		ArrayList<CFBamTZDateTypeBuff> filteredList = new ArrayList<CFBamTZDateTypeBuff>();
		CFBamTZDateTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
	...
	/**
	 *	Read a page array of the specific TZDateType buffer instances identified by the duplicate key SchemaIdx.
	 *
		...
		int revision )
	{
		...
		{
			return( (CFBamTZDateTypeBuff)cur );
		}
