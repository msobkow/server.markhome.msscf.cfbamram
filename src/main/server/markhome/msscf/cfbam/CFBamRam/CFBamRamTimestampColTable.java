...
// Description: Java 11 in-memory RAM DbIO implementation for TimestampCol.
	...
	implements ICFBamTimestampColTable
{
	...
	public CFBamRamTimestampColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTimestampColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a868" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a868" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTimestampColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTimestampColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTimestampColBuff buff;
		ArrayList<CFBamTimestampColBuff> filteredList = new ArrayList<CFBamTimestampColBuff>();
		CFBamTimestampColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TimestampCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamTimestampColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		{
			return( (CFBamTimestampColBuff)cur );
		}
		...
		return( (CFBamTimestampColBuff)editCur );
	}
		...
		CFBamTimestampColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTimestampCol().readDerivedByIdIdx( Authorization,
