...
// Description: Java 11 in-memory RAM DbIO implementation for TZDateDef.
	...
	implements ICFBamTZDateDefTable
{
	...
	public CFBamRamTZDateDefTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a83a" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTZDateDefBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTZDateDefBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTZDateDefBuff buff;
		ArrayList<CFBamTZDateDefBuff> filteredList = new ArrayList<CFBamTZDateDefBuff>();
		CFBamTZDateDefBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		int revision )
	{
		...
		return( (CFBamTZDateDefBuff)editCur );
	}
		...
		CFBamValuePKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTZDateDef().readDerivedByIdIdx( Authorization,
				cur.getRequiredTenantId(),
				cur.getRequiredId() );
			String subClassCode = cur.getClassCode();
			if( "a83a".equals( subClassCode ) ) {
