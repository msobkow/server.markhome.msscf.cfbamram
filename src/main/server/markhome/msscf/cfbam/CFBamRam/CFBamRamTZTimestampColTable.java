...
// Description: Java 11 in-memory RAM DbIO implementation for TZTimestampCol.
	...
	implements ICFBamTZTimestampColTable
{
	...
	public CFBamRamTZTimestampColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTZTimestampColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a865" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a865" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTZTimestampColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTZTimestampColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTZTimestampColBuff buff;
		ArrayList<CFBamTZTimestampColBuff> filteredList = new ArrayList<CFBamTZTimestampColBuff>();
		CFBamTZTimestampColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TZTimestampCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamTZTimestampColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		{
			return( (CFBamTZTimestampColBuff)cur );
		}
		...
		return( (CFBamTZTimestampColBuff)editCur );
	}
		...
		CFBamTZTimestampColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTZTimestampCol().readDerivedByIdIdx( Authorization,
