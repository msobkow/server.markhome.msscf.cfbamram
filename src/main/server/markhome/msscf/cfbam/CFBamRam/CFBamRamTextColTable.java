...
// Description: Java 11 in-memory RAM DbIO implementation for TextCol.
	...
	implements ICFBamTextColTable
{
	...
	public CFBamRamTextColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTextColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a866" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a866" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTextColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTextColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTextColBuff buff;
		ArrayList<CFBamTextColBuff> filteredList = new ArrayList<CFBamTextColBuff>();
		CFBamTextColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TextCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamTextColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamTextColBuff)editCur );
	}
		...
		CFBamTextColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTextCol().readDerivedByIdIdx( Authorization,
