...
// Description: Java 11 in-memory RAM DbIO implementation for TZTimeCol.
	...
	implements ICFBamTZTimeColTable
{
	...
	public CFBamRamTZTimeColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTZTimeColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a864" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a864" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTZTimeColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTZTimeColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTZTimeColBuff buff;
		ArrayList<CFBamTZTimeColBuff> filteredList = new ArrayList<CFBamTZTimeColBuff>();
		CFBamTZTimeColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TZTimeCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamTZTimeColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamTZTimeColBuff)editCur );
	}
		...
		CFBamTZTimeColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTZTimeCol().readDerivedByIdIdx( Authorization,
