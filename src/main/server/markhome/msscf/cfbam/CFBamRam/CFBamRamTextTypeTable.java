...
// Description: Java 11 in-memory RAM DbIO implementation for TextType.
	...
	implements ICFBamTextTypeTable
{
	...
	public CFBamRamTextTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTextTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a842" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a842" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTextTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTextTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTextTypeBuff buff;
		ArrayList<CFBamTextTypeBuff> filteredList = new ArrayList<CFBamTextTypeBuff>();
		CFBamTextTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TextType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamTextTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamTextTypeBuff)editCur );
	}
		...
		CFBamTextTypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTextType().readDerivedByIdIdx( Authorization,
