...
// Description: Java 11 in-memory RAM DbIO implementation for UInt16Type.
	...
	implements ICFBamUInt16TypeTable
{
	...
	public CFBamRamUInt16TypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamUInt16TypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a84b" ) ) {
		...
		long Id )
	{
		...
		CFBamUInt16TypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamUInt16TypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamUInt16TypeBuff buff;
		ArrayList<CFBamUInt16TypeBuff> filteredList = new ArrayList<CFBamUInt16TypeBuff>();
		CFBamUInt16TypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific UInt16Type buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamUInt16TypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		{
			return( (CFBamUInt16TypeBuff)cur );
		}
		...
		return( (CFBamUInt16TypeBuff)editCur );
	}
