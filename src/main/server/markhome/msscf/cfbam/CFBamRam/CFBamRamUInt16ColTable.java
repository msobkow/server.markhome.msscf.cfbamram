...
// Description: Java 11 in-memory RAM DbIO implementation for UInt16Col.
	...
	implements ICFBamUInt16ColTable
{
	...
	public CFBamRamUInt16ColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamUInt16ColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a86a" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a86a" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamUInt16ColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamUInt16ColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamUInt16ColBuff buff;
		ArrayList<CFBamUInt16ColBuff> filteredList = new ArrayList<CFBamUInt16ColBuff>();
		CFBamUInt16ColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific UInt16Col buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamUInt16ColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamUInt16ColBuff)editCur );
	}
		...
		CFBamUInt16ColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableUInt16Col().readDerivedByIdIdx( Authorization,
