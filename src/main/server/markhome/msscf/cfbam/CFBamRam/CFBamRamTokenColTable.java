...
// Description: Java 11 in-memory RAM DbIO implementation for TokenCol.
	...
	implements ICFBamTokenColTable
{
	...
	public CFBamRamTokenColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTokenColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a869" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a869" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTokenColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTokenColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTokenColBuff buff;
		ArrayList<CFBamTokenColBuff> filteredList = new ArrayList<CFBamTokenColBuff>();
		CFBamTokenColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TokenCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamTokenColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamTokenColBuff)editCur );
	}
		...
		CFBamTokenColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTokenCol().readDerivedByIdIdx( Authorization,
