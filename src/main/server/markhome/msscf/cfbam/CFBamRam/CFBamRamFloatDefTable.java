...
// Description: Java 11 in-memory RAM DbIO implementation for FloatDef.
	...
	implements ICFBamFloatDefTable
{
	...
	public CFBamRamFloatDefTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a81f" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamFloatDefBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamFloatDefBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamFloatDefBuff buff;
		ArrayList<CFBamFloatDefBuff> filteredList = new ArrayList<CFBamFloatDefBuff>();
		CFBamFloatDefBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		int revision )
	{
		...
		return( (CFBamFloatDefBuff)editCur );
	}
		...
		CFBamValuePKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableFloatDef().readDerivedByIdIdx( Authorization,
				cur.getRequiredTenantId(),
				cur.getRequiredId() );
			String subClassCode = cur.getClassCode();
			if( "a81f".equals( subClassCode ) ) {
