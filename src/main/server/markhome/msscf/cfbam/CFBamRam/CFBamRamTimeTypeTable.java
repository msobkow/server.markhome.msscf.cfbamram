...
// Description: Java 11 in-memory RAM DbIO implementation for TimeType.
	...
	implements ICFBamTimeTypeTable
{
	...
	public CFBamRamTimeTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTimeTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a845" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a845" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTimeTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTimeTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTimeTypeBuff buff;
		ArrayList<CFBamTimeTypeBuff> filteredList = new ArrayList<CFBamTimeTypeBuff>();
		CFBamTimeTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TimeType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamTimeTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamTimeTypeBuff)editCur );
	}
		...
		CFBamTimeTypeBySchemaIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableTimeType().readDerivedByIdIdx( Authorization,
