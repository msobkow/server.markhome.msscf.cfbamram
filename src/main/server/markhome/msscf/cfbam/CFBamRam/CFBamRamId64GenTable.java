...
// Description: Java 11 in-memory RAM DbIO implementation for Id64Gen.
	...
	implements ICFBamId64GenTable
{
	...
	public CFBamRamId64GenTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamId64GenBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a85b" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a85b" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamId64GenBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamId64GenBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		long SchemaDefId )
	{
		...
		CFBamId64GenBuff buff;
		ArrayList<CFBamId64GenBuff> filteredList = new ArrayList<CFBamId64GenBuff>();
		CFBamId64GenBuff[] buffList = readDerivedBySchemaIdx( Authorization,
			TenantId,
			SchemaDefId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
	...
	/**
	 *	Read a page array of the specific Id64Gen buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamId64GenBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
	...
	/**
	 *	Read a page array of the specific Id64Gen buffer instances identified by the duplicate key DispIdx.
	 *
	 *	@param	Authorization	The session authorization information.
	 *
	 ...
	 *	@throws	CFLibNotSupportedException thrown by client-side implementations.
	 */
	public CFBamId64GenBuff[] pageBuffByDispIdx( CFSecAuthorization Authorization,
		Long DispenserTenantId,
		Long DispenserId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamId64GenBuff)editCur );
	}
		...
		CFBamId64GenByDispIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableId64Gen().readDerivedByIdIdx( Authorization,
