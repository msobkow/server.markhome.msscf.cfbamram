...
// Description: Java 11 in-memory RAM DbIO implementation for Id16Gen.
	...
	implements ICFBamId16GenTable
{
	...
	public CFBamRamId16GenTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamId16GenBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a859" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a859" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamId16GenBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamId16GenBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		long SchemaDefId )
	{
		...
		CFBamId16GenBuff buff;
		ArrayList<CFBamId16GenBuff> filteredList = new ArrayList<CFBamId16GenBuff>();
		CFBamId16GenBuff[] buffList = readDerivedBySchemaIdx( Authorization,
			TenantId,
			SchemaDefId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
	...
	/**
	 *	Read a page array of the specific Id16Gen buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamId16GenBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
	...
	/**
	 *	Read a page array of the specific Id16Gen buffer instances identified by the duplicate key DispIdx.
	 *
	 *	@param	Authorization	The session authorization information.
	 *
	 ...
	 *	@throws	CFLibNotSupportedException thrown by client-side implementations.
	 */
	public CFBamId16GenBuff[] pageBuffByDispIdx( CFSecAuthorization Authorization,
		Long DispenserTenantId,
		Long DispenserId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamId16GenBuff)editCur );
	}
		...
		CFBamId16GenByDispIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableId16Gen().readDerivedByIdIdx( Authorization,
