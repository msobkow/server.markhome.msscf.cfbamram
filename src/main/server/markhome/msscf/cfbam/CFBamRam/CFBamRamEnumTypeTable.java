...
// Description: Java 11 in-memory RAM DbIO implementation for EnumType.
	...
	implements ICFBamEnumTypeTable
{
	...
	public CFBamRamEnumTypeTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamEnumTypeBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a857" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a857" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamEnumTypeBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamEnumTypeBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamEnumTypeBuff buff;
		ArrayList<CFBamEnumTypeBuff> filteredList = new ArrayList<CFBamEnumTypeBuff>();
		CFBamEnumTypeBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific EnumType buffer instances identified by the duplicate key SchemaIdx.
	 *
	 ...
	 */
	public CFBamEnumTypeBuff[] pageBuffBySchemaIdx( CFSecAuthorization Authorization,
		long TenantId,
		long SchemaDefId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamEnumTypeBuff)editCur );
	}
		...
		CFBamEnumTypeBuff Buff )
	{
		...
		Object arrCheckTags[] = schema.getTableEnumTag().readDerivedByEnumIdx( Authorization,
						existing.getRequiredTenantId(),
						existing.getRequiredId() );
		if( arrCheckTags.length > 0 ) {
