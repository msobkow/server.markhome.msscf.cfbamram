...
// Description: Java 11 in-memory RAM DbIO implementation for TimeCol.
	...
	implements ICFBamTimeColTable
{
	...
	public CFBamRamTimeColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamTimeColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a867" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a867" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamTimeColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamTimeColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamTimeColBuff buff;
		ArrayList<CFBamTimeColBuff> filteredList = new ArrayList<CFBamTimeColBuff>();
		CFBamTimeColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific TimeCol buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamTimeColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamTimeColBuff)editCur );
	}
