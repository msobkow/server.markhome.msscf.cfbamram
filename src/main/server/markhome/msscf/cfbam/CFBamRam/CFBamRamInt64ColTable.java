...
// Description: Java 11 in-memory RAM DbIO implementation for Int64Col.
	...
	implements ICFBamInt64ColTable
{
	...
	public CFBamRamInt64ColTable( ICFBamSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFBamInt64ColBuff Buff )
	{
		...
		if( Buff.getClassCode().equals( "a85e" ) ) {
		...
		CFBamValuePKey PKey )
	{
		...
		if( ( buff != null ) && ( ! buff.getClassCode().equals( "a85e" ) ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		long Id )
	{
		...
		CFBamInt64ColBuff buff = readDerivedByIdIdx( Authorization,
			TenantId,
			Id );
		...
		String Name )
	{
		...
		CFBamInt64ColBuff buff = readDerivedByUNameIdx( Authorization,
			TenantId,
			ScopeId,
			Name );
		if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long TenantId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByValTentIdx( Authorization,
			TenantId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && buff.getClassCode().equals( "a809" ) ) {
		...
		long ScopeId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByScopeIdx( Authorization,
			TenantId,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long DefSchemaId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaTenantId,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByPrevIdx( Authorization,
			PrevTenantId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long NextId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByNextIdx( Authorization,
			NextTenantId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		Long PrevId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByContPrevIdx( Authorization,
			TenantId,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
		...
		Long NextId )
	{
		...
		CFBamInt64ColBuff buff;
		ArrayList<CFBamInt64ColBuff> filteredList = new ArrayList<CFBamInt64ColBuff>();
		CFBamInt64ColBuff[] buffList = readDerivedByContNextIdx( Authorization,
			TenantId,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
	...
	/**
	 *	Read a page array of the specific Int64Col buffer instances identified by the duplicate key TableIdx.
	 *
	 ...
	 */
	public CFBamInt64ColBuff[] pageBuffByTableIdx( CFSecAuthorization Authorization,
		long TenantId,
		long TableId,
		Long priorTenantId,
		Long priorId )
		...
		int revision )
	{
		...
		return( (CFBamInt64ColBuff)editCur );
	}
		...
		CFBamInt64ColByTableIdxKey argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = schema.getTableInt64Col().readDerivedByIdIdx( Authorization,
